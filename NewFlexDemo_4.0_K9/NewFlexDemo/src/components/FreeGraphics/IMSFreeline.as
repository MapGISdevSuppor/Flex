package components.FreeGraphics
{
	 ...
	 * 
	 */	
	public class IMSFreeline extends GraphicsBase
	{
		//-----------------------------------------------------------------内部成员--------------------------------------------------------------
		/**
		 * 存储坐标数组 
		 ...
		 * 
		 */		
		public function IMSFreeline(coordinateType:int=CoordinateType.Screen)
		{
			_tmpLine.addEventListener(MouseEvent.MOUSE_DOWN,mouseDownFn);
			_tmpLine.addEventListener(MouseEvent.MOUSE_UP,mouseUpFn);
			super(coordinateType);
			_shape.addChild(_tmpLine);
			lineStyle(2,0xFF0000);
		}
		
		private function mouseDownFn(e:MouseEvent):void{
			var g:Graphics = _tmpLine.graphics;
			g.endFill();
			g.moveTo(e.localX,e.localY);//mouseX,mouseY);
			g.lineStyle(2,0xff0000,1);
			_tmpLine.addEventListener(MouseEvent.MOUSE_MOVE,mouseMoveFn);
		}
		
		private function mouseMoveFn(e:MouseEvent):void{
			var g:Graphics = _tmpLine.graphics;
			g.lineTo(e.localX,e.localY);
		}
		private function mouseUpFn(e:MouseEvent):void{
			_tmpLine.removeEventListener(MouseEvent.MOUSE_MOVE,mouseMoveFn);
		}
		//-------------------------------------------------------------------公有方法----------------------------------------------------------------
		/**
		 * 清除绘制折线
